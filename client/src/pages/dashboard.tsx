...
function EvlfrqAccessStatus() {
  const { data: accessStatus, isLoading: statusLoading, refetch } = useQuery({
  ...
  const handleActivateKey = async () => {
    ...
    try {
      const response = await apiRequest("POST", "/api/evlfrq/activate-key", { key: activationKey });
      const data = await response.json();
      setActivationKey("");
      setShowActivationForm(false);
      refetch();
    } catch (error: any) {
      alert(error.message || "Greška pri aktivaciji ključa");
    } finally {
  ...
  return (
    <Card data-testid="card-evlfrq-access">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <CheckCircle2 className={`w-5 h-5 ${accessStatus?.hasAccess ? "text-green-500" : "text-gray-400"}`} />
          EVLFRQ Audio Analyzer
        </CardTitle>
        <CardDescription>
          Eksklusivni audio analitički alat sa vremenskim ograničenjem pristupa
        </CardDescription>
      </CardHeader>
          ...
          <div className="bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg p-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">
                {accessStatus.durationDays} dana licenca
              </Badge>
            </div>
            
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="space-y-1">
                <p className="text-muted-foreground">Preostalo</p>
                <p className="font-semibold text-green-700 dark:text-green-300 text-lg">
                  {accessStatus.daysRemaining > 0 
                    ? `${accessStatus.daysRemaining} ${accessStatus.daysRemaining === 1 ? 'dan' : 'dana'}`
                    : `${accessStatus.hoursRemaining} ${accessStatus.hoursRemaining === 1 ? 'sat' : 'sati'}`
                  }
                </p>
              </div>
              <div className="space-y-1">
                ...
                <p className="font-medium text-green-700 dark:text-green-300">
                  {new Date(accessStatus.expiresAt).toLocaleDateString("sr-RS", {
                    day: "2-digit",
                    month: "short",
                    year: "numeric"
                  })}
            ...
            {accessStatus.activatedAt && (
              <div className="text-xs text-green-600 dark:text-green-400 border-t border-green-200 dark:border-green-800 pt-2">
                Aktivirano: {new Date(accessStatus.activatedAt).toLocaleDateString("sr-RS", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                  hour: "2-digit",
            ...
            <Link href="/audio-analyzer">
              <Button className="w-full" data-testid="button-go-to-analyzer">
                Otvori EVLFRQ Analyzer
              </Button>
            </Link>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              <p className="text-sm text-blue-900 dark:text-blue-100">
                Nemate aktivnog pristupa EVLFRQ audio analizatoru. Unesite aktivacijski ključ da biste dobili pristup.
              </p>
            </div>
            ...
            {!showActivationForm ? (
              <Button 
                ...
                variant="outline"
                className="w-full"
                data-testid="button-show-activation-form"
              >
                Aktiviraj Ključ
              </Button>
            ) : (
              <div className="space-y-2">
                <Input
                  type="password"
                  placeholder="Unesite 64-karakterni ključ..."
                  value={activationKey}
                  onChange={(e) => setActivationKey(e.target.value)}
                  data-testid="input-evlfrq-activation-key"
                />
                <div className="flex gap-2">
                  ...
                  >
                    {isActivating ? "Aktivacija..." : "Aktiviraj"}
                  </Button>
                  <Button
                    onClick={() => {
                      setShowActivationForm(false);
