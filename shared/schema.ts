...
// EVLFRQ Auth Keys - exclusive audio analyzer access with time-limited activation
export const evlfrqAuthKeys = pgTable("evlfrq_auth_keys", {
  id: serial("id").primaryKey(),
  key: varchar("key", { length: 64 }).notNull().unique(), // 64-character random key
  durationDays: integer("duration_days").notNull(), // How long the access lasts (7, 30, 90 days, etc)
  createdBy: integer("created_by").notNull().references(() => users.id), // Admin who created it
  createdAt: timestamp("created_at").defaultNow().notNull(),
  usedBy: integer("used_by").references(() => users.id), // User who activated the key
  usedAt: timestamp("used_at"), // When the key was activated
  expiresAt: timestamp("expires_at"), // When access expires (calculated when used)
  isUsed: boolean("is_used").notNull().default(false), // Has this key been used?
}, (table) => ({
  // Index for finding active/unused keys
  isUsedIdx: index("evlfrq_keys_is_used_idx").on(table.isUsed),
  // Index for finding keys by user who used it
  usedByIdx: index("evlfrq_keys_used_by_idx").on(table.usedBy),
  // Index for finding keys by creation admin
  createdByIdx: index("evlfrq_keys_created_by_idx").on(table.createdBy),
  // Index for checking key expiration
  expiresAtIdx: index("evlfrq_keys_expires_at_idx").on(table.expiresAt),
}));

export const insertEvlfrqAuthKeySchema = createInsertSchema(evlfrqAuthKeys).omit({
  id: true,
  createdAt: true,
  key: true,
  createdBy: true,
  ...
  durationDays: z.number().min(1, "Najmanje 1 dan").max(365, "Maksimalno 365 dana"),
});

export type InsertEvlfrqAuthKey = z.infer<typeof insertEvlfrqAuthKeySchema>;
export type EvlfrqAuthKey = typeof evlfrqAuthKeys.$inferSelect;

// EVLFRQ Security Logs - audit trail for security events
export const evlfrqSecurityLogs = pgTable("evlfrq_security_logs", {
  id: serial("id").primaryKey(),
  eventType: varchar("event_type", { length: 50 }).notNull(),
  userId: integer("user_id").references(() => users.id),
  ipAddress: varchar("ip_address", { length: 45 }),
  keyId: integer("key_id").references(() => evlfrqAuthKeys.id),
  details: text("details"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => ({
  eventTypeIdx: index("evlfrq_security_event_type_idx").on(table.eventType),
  userIdx: index("evlfrq_security_user_idx").on(table.userId),
  ipIdx: index("evlfrq_security_ip_idx").on(table.ipAddress),
  createdIdx: index("evlfrq_security_created_idx").on(table.createdAt),
}));

export type EvlfrqSecurityLog = typeof evlfrqSecurityLogs.$inferSelect;
